...
export function PolicyScenariosChart({ scenarios, selectedScenario = null }) {
  ...
  const scenarioKeys = Object.keys(scenarios).filter(key => key !== 'baseline');
  const scenarioLabels = scenarioKeys.map(key => 
    scenarios[key].description.replace(/^[^:]*:\s*/, '') // Remove prefix before colon
  );
  const revenueChanges = scenarioKeys.map(key => 
    scenarios[key].revenue_change_from_baseline / 1000000000 // Convert to billions
  );
    ...
    data: [
      {
        ...
        text: revenueChanges.map(val => 
          `${val >= 0 ? '+' : ''}£${val.toFixed(1)}bn`
        ),
        textposition: 'auto',
        hovertemplate: '<b>%{x}</b><br>Revenue change: %{text}<br><i>Compared to current system</i><extra></extra>',
      }
    ],
    layout: {
...
export function SectoralImpactChart({ sectoralData }) {
  ...
  const baselineImpacts = sectors.map(sector => 
    sectoralData.sectoral_impacts[sector].scenario_impacts.baseline.total_vat_liability / 1000000
  );
  
  const splitRateImpacts = sectors.map(sector => 
    sectoralData.sectoral_impacts[sector].scenario_impacts.split_rate.total_vat_liability / 1000000
  );

  const isLabourIntensive = sectors.map(sector => 
    sectoralData.sectoral_impacts[sector].is_labour_intensive
  );
    ...
    data: [
      {
        ...
        hovertemplate: '<b>%{x}</b><br>VAT liability: £%{y:.1f}m<br><i>Under current 20% rate</i><extra></extra>',
      },
      {
        type: 'bar',
        x: sectorLabels,
        y: splitRateImpacts,
        marker: {
          color: isLabourIntensive.map(intensive => intensive ? TEAL_ACCENT : BLUE_LIGHT),
        },
        name: 'Split rate system',
        hovertemplate: '<b>%{x}</b><br>VAT liability: £%{y:.1f}m<br><i>Under split rate system</i><extra></extra>',
      }
    ],
    layout: {
...
export function FirmDistributionChart({ firmData }) {
  if (!firmData || !firmData.sectors) {
    return <div className="chart-container">No firm distribution data available.</div>;
  }

  // Aggregate firm counts by turnover band across all sectors
  const turnoverBands = ["£0-30k", "£30-60k", "£60-90k", "£90-120k", "£120-200k", "£200k+"];
...
export function ThresholdEffectsChart({ thresholdData }) {
  if (!thresholdData || !thresholdData.threshold_bunching) {
    return <div className="chart-container">No threshold effects data available.</div>;
  }
    ...
    data: [
      {
        ...
        marker: {
          color: ranges.map(range => 
            range.includes('80k-90k') ? ORANGE_ACCENT : 
            range.includes('90k-100k') ? DARK_GRAY : BLUE_PRIMARY
          ),
        },
        text: firmCounts.map(count => count.toLocaleString()),
        textposition: 'auto',
        hovertemplate: '<b>%{x}</b><br>Firms: %{y:,}<br><i>Firms in this turnover range</i><extra></extra>',
      }
    ],
    layout: {
...
export function VATElasticityRevenueChart({ scenarios, elasticityRange }) {
  if (!scenarios || !elasticityRange) {
    return <div className="chart-container">No elasticity revenue data available.</div>;
  }

  // Generate elasticity values across the range
  const elasticityStep = (elasticityRange.max - elasticityRange.min) / 10;
  const elasticityValues = Array.from({ length: 11 }, (_, i) => 
    elasticityRange.min + (i * elasticityStep)
  );
  ...
  const traces = scenarioKeys.map((scenarioKey, index) => {
    ...
    const colors = [BLUE_PRIMARY, TEAL_ACCENT, GREEN_ACCENT, ORANGE_ACCENT, DARK_GRAY];
    ...
    return {
      ...
      hovertemplate: '<b>%{fullData.name}</b><br>Elasticity: %{x:.1f}<br>Revenue: £%{y:.1f}bn<extra></extra>'
    };
  });
...
export function VATRegistrationChart() {
  ...
  const businessSizes = ['Micro (0-9)', 'Small (10-49)', 'Medium (50-249)', 'Large (250+)'];
  const registrations = [
    ...
    data: [
      {
        ...
        textposition: 'auto',
        hovertemplate: '<b>%{x}</b><br>VAT Registrations: %{y:,}<extra></extra>',
      }
    ],
    layout: {
...
export function VATRateComparisonChart() {
  ...
  const countries = ['UK', 'Germany', 'France', 'Italy', 'Spain', 'Netherlands', 'Belgium', 'Austria'];
