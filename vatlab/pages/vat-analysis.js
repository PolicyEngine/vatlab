            ...
            {analysisResults ? (
              <div>
                <p style={{ fontSize: '0.95rem', lineHeight: 1.5, marginBottom: '1rem' }}>
                  <strong>Baseline:</strong> {analysisResults.useCurrentUKPolicy ? 
                    '£90,000 registration threshold with 20% standard VAT rate for all sectors. No graduated threshold system, limited reduced rates (5% for energy-saving materials).' :
                    `£${analysisResults.threshold?.toLocaleString()} registration threshold with ${analysisResults.fullRateLaborIntensive}% rate for labor-intensive sectors (${
                      analysisResults.laborIntensiveIndustries?.length > 0 
                        ? analysisResults.laborIntensiveIndustries.map(industry => 
                ...
                <p style={{ fontSize: '0.95rem', lineHeight: 1.5 }}>
                  <strong>Reform:</strong> £{analysisResults.threshold?.toLocaleString()} registration threshold{analysisResults.graduatedEndThreshold && ` graduating to £${analysisResults.graduatedEndThreshold.toLocaleString()}`} with {analysisResults.fullRateLaborIntensive}% rate for labor-intensive sectors ({
                    analysisResults.laborIntensiveIndustries?.length > 0 
                      ? analysisResults.laborIntensiveIndustries.map(industry => 
                          industry.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())
                        ).join(', ')
